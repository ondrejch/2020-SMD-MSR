...
%RelP0 = 0.01; %Relative initial power

P_MSRE = 8; %Operational power of MSRE [MW]

Psf_li = P/P_MSRE; %Cube scaling factor
Psf_sq = power(Psf_li,1); %Square scaling factor
...
n_frac0 = 1*RelP0 ; % initial fractional nuetron density n/n0 [n/cm^3/s]
...
% Core Upflow
v_g      = Psf_li*1.95386; % graphite volume(m^3) ORNL-TM-0728 p. 101
...
hA_fg    = Psf_sq*((0.02*9/5)/nn_f); % (fuel to graphite heat transfer coeff x heat transfer area) (MW/�C) ORNL-TM-1647 p.3, TDAMSRE p.5
...
%%%With Rel initial power
Tf_in = (Tf0_hot +Tf0_cold)/2 +  RelP0 * (Tf0_cold-(Tf0_hot +Tf0_cold)/2); 
T0_f2 = (Tf0_hot +Tf0_cold)/2 +  RelP0 * (Tf0_hot-(Tf0_hot +Tf0_cold)/2);     

% T0_f1  = 6.405952380952389E+02; %in �C
% T0_g1  = 6.589285714285924E+02; %in �C 

T0_f1  = Tf_in + (T0_f2-Tf_in)/2; % 6.405952380952389e+02; in �C
T0_g1  = T0_f1+(k_g*(P*RelP0)/hA_fg); % 6.589285714285924e+02; in �C 
...
hA_pn    = (Psf_sq*ha_p)/nn_s; %3.030; % (primary to tube heat transfer coeff x heat transfer area) in MW/�C
...
hA_sn    = (Psf_sq*ha_s)/nn_s; % (tube to secondary heat transfer coeff x heat transfer area) in MW/�C
...
%%%With Rel initial power
Ts_in = (Ts0_hot +Ts0_cold)/2 +  RelP0 * (Ts0_cold-(Ts0_hot +Ts0_cold)/2); 
T0_s4 = (Ts0_hot +Ts0_cold)/2 +  RelP0 * (Ts0_hot-(Ts0_hot +Ts0_cold)/2);  
...
%%%With Rel initial power
Trs_in = (Tr0_hot +Tr0_cold)/2 +  RelP0 * (Tr0_cold-(Tr0_hot +Tr0_cold)/2); 
T0_rs = (Tr0_hot +Tr0_cold)/2 +  RelP0 * (Tr0_hot-(Tr0_hot +Tr0_cold)/2); 
...
hA_rpn    = (Psf_sq*h_roverall*A_rad)/nn_rs; %3.030; % (primary to secondary heat transfer coeff x heat transfer area) in MW/�C

% Secondary Side - DONE
mcp_rsn   = mn_rs*scp_rs; % (mass of material x heat capacity of material) of coolant salt per lump in MW-s/�C
hA_rsn = (Psf_sq*h_roverall*A_rad)/nn_rs; % (tube to secondary heat transfer coeff x heat transfer area) in MW/�C
...
DHG1_0 = 0.025125*RelP0;
DHG2_0 = 0.0198578*RelP0;
DHG3_0 = 0.0227606*RelP0;
...
lam_bubl = 0;    % effective bubbling out constant (s^-1)
