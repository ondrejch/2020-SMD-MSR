...
%% Transient - 4
%%% Runs a simulation of UHX failure with DHRS open, both pump trips & SCRAM simultaniously
%%% Results are plotted in three panels; Reactor power, Core tempratures & feedbacks
%%% Simulation done in six steps

%%% Step - 1; Simulation is run for 2000[s] at 500[Mw_t]
%%% Step - 2; Using UHX_MODE = 2, Radiator tripped at 2000[s]
%%% Step - 3; Using DHRS_MODE = 1, Normal DHRS open at 2000[s]
%%% Step - 4; Both primary and secondary pumps are tripped at 2000[s]
%%% Step - 5; -1400[pcm] worth reactivity inserted as a step at 2000[s]
%%% Step - 6; Simulation continued till 15000[s]

%% User Inputs

%%% Basic Simulation Parameters
simtime = 15000;                                                            %Simulation time [s]
ts_max = 1e-2;                                                             %Maximum timestep [s] 
P=500;                                                                       %Operational thermal power [MW]
...
DHRS_Power = P*(0.040);                                                     %Maximum power that can be removed by DHRS
Power_Bleed = P*(0.001);                                                    %Some power will removed from DRACS even when its not used 
...
saveas(gcf,'Transient_4.png')
savefig('Transient_4.fig')
