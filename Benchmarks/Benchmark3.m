...
%% Benchmark Transient - 3 --> +24.8[pcm] & +14.8 [pcm] for U233 MSRE @ 8MW
%%% Runs step insertion value +24.8[pcm] & +14.8 [pcm] at 8[MW_t] for U233 MSRE
%%% Results are ploted along with MSRE one region, nine region and MSRE experimental results
%%% Simulation done in four steps in a for loop
...
%%% Step - 4; +24.8[pcm] & +14.8 [pcm] worth reactivity is inserted as a step at 5000[s]

%% Benchmark 1 - Inputs

i = 2;
    
rho_abs = [+24.8E-5 +14.8E-5];
...
reactdata = [0 rho_abs(i) rho_abs(i)];                                     %Reactivity insertions [abs]
...
xlabel('Time [s]')
ylabel('\Delta P [MW]')
legend('1 Region MSRE +24.8[pcm]', 'Experiment', '9 Region MSRE +24.8[pcm]', '1 Region MSRE +14.8[pcm]', '9 Region MSRE +14.8[pcm]', 'SMD MSR +24.8[pcm]','SMD MSR +14.8[pcm]')
...
saveas(gcf,'Exp8[MW]_benchmark.png')
savefig('Exp8[MW]_benchmark.fig')
